(()=>{
 ...
 const account=root.querySelector('[data-account-form]'),request=root.querySelector('[data-payout-form]'),status=root.querySelector('[data-payout-status]'),saved=root.querySelector('[data-account-saved]'),history=root.querySelector('[data-payout-history]');let token='',busy=false;
 ...
 account.addEventListener('submit',e=>{e.preventDefault();call(account);});request.addEventListener('submit',e=>{e.preventDefault();if(confirm(`${money(request.elements.coins.value)}코인, ${money(Number(request.elements.coins.value)*19000)}원 출금을 신청할까요?`))call(request);});
 request.elements.coins.addEventListener('input',()=>root.querySelector('[data-payout-amount]').textContent=money(Number(request.elements.coins.value)*19000)+'원');document.querySelector('[data-wallet-open]')?.addEventListener('click',()=>call());
})();
