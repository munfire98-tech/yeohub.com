...
function ms_data(): array {
    ...
    foreach($members as $id=>$member){
        ...
        if(mg_referrer($member,$members)===$uid)mg_reconcile((string)$id);
    }
    ...
    return ['available'=>true,'me'=>$me,'rows'=>$rows,'ledger'=>$ledger,'balance'=>$balance,'pending'=>$pending,'accepted'=>$accepted];
}
function ms_action(string $uid,array $m,string $action,string $label): void {
    ?><form action="/manager_portal.php" method="post"><input type="hidden" name="csrf" value="<?=mg_e($_SESSION['csrf'])?>"><input type="hidden" name="action" value="decide"><input type="hidden" name="target" value="<?=mg_e($uid)?>"><input type="hidden" name="request_key" value="<?=mg_e(mg_link_key($uid,$m))?>"><input type="hidden" name="return_to" value="clients"><input type="hidden" name="return_month" value="<?=mg_e((string)($_GET['m']??''))?>"><input type="hidden" name="return_type" value="<?=mg_e((string)($_GET['type']??''))?>"><button class="ms-button <?=$action==='accepted'?'ms-button--primary':''?>" name="decision" value="<?=mg_e($action)?>"><?=mg_e($label)?></button></form><?php
}
function ms_render(array $data): void {
    ...
    <div class="ms-user-list"><?php if(!$data['accepted']): ?><div class="ms-empty">아직 연결된 담당 유저가 없습니다.<br>연결 요청은 알림 탭에서 확인하세요.</div><?php endif; ?>
    ...
    <div class="ms-content" id="ms-notifications" role="tabpanel" aria-labelledby="ms-tab-notifications" hidden>
      ...
      <article class="ms-user" data-notification-key="<?=mg_e(mg_link_key((string)$uid,$m))?>">
        <div class="ms-user-head"><strong><?=mg_e($m['nickname']??$uid)?></strong><span class="ms-badge ms-badge--pending">수락 대기</span></div>
...
function ms_render_wallet(array $data):void {if(empty($data['available']))return; ?>
    <dialog id="manager-wallet-dialog" aria-labelledby="manager-wallet-title"><div class="wallet-dialog-head"><h2 id="manager-wallet-title">파이어코인</h2><button type="button" data-wallet-close aria-label="닫기">×</button></div><div class="wallet-dialog-body"><div class="ms-wallet"><span><?=mg_icon('coin')?> 파이어코인</span><strong><?=$data['balance']?><small>개</small></strong><p>출금 가능한 코인</p></div><section id="manager-payout" aria-label="계좌 및 출금"><h3>출금 신청</h3><p>1코인 = 19,000원 · 관리자 확인 후 직접 송금</p><p data-payout-balance>출금 가능 잔액 확인 중</p><p data-payout-deficit></p><p data-account-saved></p><details><summary>정산 계좌 등록 · 변경</summary><form data-account-form><input type="hidden" name="action" value="account"><label>은행명<input name="bank" maxlength="40" required autocomplete="off"></label><label>예금주<input name="holder" maxlength="60" required autocomplete="off"></label><label>계좌번호<input name="number" inputmode="numeric" maxlength="30" required autocomplete="off"></label><p>계좌번호와 예금주를 정확히 입력해 주세요. 계좌 실명 검증은 제공하지 않습니다. 변경된 계좌는 다음 신청부터 적용됩니다.</p><button>계좌 저장</button></form></details><form data-payout-form><input type="hidden" name="action" value="request"><label>신청 코인 수<input name="coins" type="number" min="1" step="1" value="1" required></label><p>신청 금액 <strong data-payout-amount>19,000원</strong></p><button disabled>출금 신청</button></form><p data-payout-status role="status"></p><h3>출금 내역</h3><div data-payout-history></div></section><div class="ms-ledger-title">적립 내역</div>
    <?php if(!$data['ledger']): ?><div class="ms-empty">아직 적립 내역이 없습니다.</div><?php endif; ?>
    <?php foreach($data['ledger'] as $uid=>$r): ?><div class="ms-ledger-row"><div><strong><?=mg_e($uid)?></strong><small><?=mg_e(substr($r['at'],0,10))?></small></div><span><?=empty($r['reversed'])?'+1':'회수됨'?></span></div><?php endforeach; ?>
    <p class="ms-policy">첫 코드 등록 후 최초 실제 결제에 1개가 적립됩니다. 테스트 결제·중복 결제는 제외되며, 해당 결제의 전액 환불 시 회수됩니다.</p></div></dialog><?php
}
