...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['act'] ?? '') === 'lookup') {
  ...
  foreach ($titles as $t) {
    ...
    $dongList[] = [
      ...
      'arch_area' => is_numeric($t['archArea'] ?? '') && (float)$t['archArea'] >= 0 ? trim((string)$t['archArea']) : '',
  ...
  $patch = [
    ...
    'bd_area_arch' => ($head && is_numeric($head['archArea'] ?? '') && (float)$head['archArea'] >= 0) ? trim((string)$head['archArea']) : '',
...
var STEPS = [
  ...
  { field:'__areas', q:'면적은 어떻게 되나요?', type:'areas',
    hint:'연면적과 건축면적은 건축물대장 값을 확인해 주세요. 바닥면적은 별도 항목이며, 모르면 비워두셔도 됩니다.',
    dontknow:'area' },
...
function filled(s){
  ...
  if (s.field === '__areas')  return String(SAVED.area_t||'').trim() !== '' || String(SAVED.area_f||'').trim() !== '' || String(SAVED.bd_area_arch||'').trim() !== '';
...
function ask(s){
  ...
  if (s.type === 'areas'){
    ...
    var areaInputs={};
    [['area_t','연면적','예: 8500'],['bd_area_arch','건축면적','예: 900.25'],['area_f','바닥면적 (선택)','예: 850']].forEach(function(p){
      var lb=document.createElement('label');
      lb.innerHTML='<span>'+esc(p[1])+' (㎡)</span>';
      var i=document.createElement('input');
      i.type='number'; i.min='0'; i.step='any'; i.inputMode='decimal'; i.placeholder=p[2];
      i.value = SAVED[p[0]] || '';
    ...
    var areaGo = mkGo(b, function(){
      ...
      var arch=(areaInputs.bd_area_arch.value||'').trim();
      if(Object.values(areaInputs).some(function(input){return !input.reportValidity();}))return;
      if(total==='' && floor==='' && arch===''){ areaInputs.area_t.focus(); return; }
      var patch={area_t:total || (SAVED.area_t || ''), area_f:floor || (SAVED.area_f || ''), bd_area_arch:arch};
      var shown=[];
      if(patch.area_t!=='') shown.push('연면적 '+Number(patch.area_t).toLocaleString()+'㎡');
      if(patch.bd_area_arch!=='') shown.push('건축면적 '+Number(patch.bd_area_arch).toLocaleString()+'㎡');
    ...
    areaInputs.bd_area_arch.addEventListener('keydown', function(e){ if(e.key==='Enter'){ e.preventDefault(); areaGo.click(); } });
...
function finish(){
  ...
  typing(function(){
    ...
    var rows=[
      ...
      ['건축면적', SAVED.bd_area_arch ? Number(SAVED.bd_area_arch).toLocaleString()+'㎡' : ''],
